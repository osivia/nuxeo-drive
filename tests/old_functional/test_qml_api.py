...
class TestDownload(OneUserTest):
    ...
    def test_api_resume_download(self):
        """
        Pause the transfer by simulating a click on the pause/resume icon
        on the current download in the systray menu.
        """
        ...
        # A tmp file has been created
        assert len(list(dao.get_downloads())) == 1
        download = list(dao.get_downloads())[0]
        assert Path(download.tmpname).exists()

        # Resume the download using api resume_transfer
        self.app.manager = self.manager_1
        self.app.manager.dao = dao
        self.app.open_authentication_dialog = Mock()

        my_api = QMLDriveApi(self.app)
        my_api.resume_transfer(
            "download", "fake_engine_uid", list(dao.get_downloads())[0].uid
        )
        assert not list(dao.get_downloads())
        assert not Path(download.tmpname).exists()
